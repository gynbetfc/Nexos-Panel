...
print(f"""
🔑 ID: {DEVICE_ID}
🔗 Site: https://nexos-panel.onrender.com

📌 SALVE SEU ID! Use o site para rastrear.
""")
print("="*45)
...
def executar_comando(acao):
    if acao == "vibrar": run("termux-vibrate -d 1000", timeout=3)
        ...
        # Toca som padrão do sistema
        run("am start -a android.intent.action.VIEW -d file:///system/media/audio/ringtones/Andromeda.ogg -t audio/*", timeout=3)
...
while True:
    ...
    if resp and resp.get("status") == "success":
        ...
        # Terminal minimalista - UMA LINHA
        print(f"\r{icone} {datetime.now().strftime('%H:%M:%S')} | Bat:{bat}% | {rede} | {'✅ GPS' if gps_ok else '📡'}   ", end="", flush=True)
